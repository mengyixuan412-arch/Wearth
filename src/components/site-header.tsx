...
export default function SiteHeader() {
  const goToAnchor = (anchor: string) => scrollEnv.lenisScrollTo(anchor, { lerp: 0.1 });
  ...
  return (
    <header className="flex lg:flex-row flex-col justify-between items-start gap-4 lg:gap-6">
      <div className="flex flex-col gap-4 lg:gap-10 min-w-0">
        <DottedLink href="/" dotted className={`${TITLE_FACE} p-2 text-l1 text-sm lg:text-lg`} style={TITLE_WIDTH}>
          {SUBMARK}
        </DottedLink>
      ...
      <nav className="flex flex-wrap lg:justify-end gap-x-3 lg:gap-x-8 gap-y-1 lg:gap-y-2 w-full lg:w-auto font-mono-2">
        {NAV_ITEMS.map((item) => (
          <button
            key={item.anchor}
            type="button"
            onClick={() => goToAnchor(item.anchor)}
